...
impl Renderer {
    ...
    /// Like [`present`], but returns the [`CommandBuffer`] without submitting.
    ///
    /// Use this when integrating with an external renderer (e.g. Bevy) that
    /// manages its own GPU submission pipeline. Call this each frame — it
    /// automatically reclaims staging belt buffers from the previous frame.
    pub fn present_to_buffer(
        ...
        // Reclaim staging belt buffers from previous frame's submission.
        self.staging_belt.recall();
    ...
    /// Render into an externally-provided [`CommandEncoder`].
    ///
    /// Use this when the host renderer (e.g. Bevy) needs Iced's commands
    /// encoded into its own encoder rather than a separate command buffer.
    /// Call this each frame — it automatically reclaims staging belt buffers
    ...
    pub fn render_to_encoder(
        ...
        self.prepare(encoder, viewport);
        self.render(encoder, frame, clear_color, viewport);
